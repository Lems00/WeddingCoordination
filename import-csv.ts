export async function handleResetTasks(env: any) {
  const statements = [
  ...
  const tasksData = [
    ...
    // ... tout le reste du tableau ici ...
  ];

  const insertQuery = "INSERT INTO tasks (id, phase, category, name, duration, start_date, end_date, predecessor, assignee, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

  for (const row of tasksData) {
    statements.push(env.DB.prepare(insertQuery).bind(...row));
  }

  try {
    await env.DB.batch(statements);
    return "Remplacement réussi !";
  } catch (error: any) {
