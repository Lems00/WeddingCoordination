  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left Column - Fixed (reduced on mobile) */}
        <div className="w-32 sm:w-40 md:w-56 flex-shrink-0 flex flex-col border-r border-slate-100">
          {/* Header */}
          <div className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 bg-slate-50 border-b border-slate-200 flex-shrink-0">
            <span className="text-[10px] sm:text-xs font-semibold text-slate-500 uppercase tracking-wider">Tâche</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-3 [&::-webkit-scrollbar-track]:bg-slate-50 [&::-webkit-scrollbar-thumb]:bg-indigo-500 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-indigo-600">
            {PHASES.map((phase) => {
              ...
              return (
                <div key={phase}>
                  {/* Phase header */}
                  <div className="bg-slate-50/50 border-b border-slate-100 px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 sticky top-0 z-5">
                    <span className={cn("text-[10px] sm:text-xs font-bold uppercase tracking-wider",
                  ...
                  {/* Tasks in phase */}
                  {phaseTasks.map((t) => (
                    <div key={t.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition px-1.5 sm:px-2 md:px-3 py-1.5 sm:py-2 flex items-center gap-1">
                      <button
                        onClick={() => toggleTaskExpand(t.id)}
                        className="flex-shrink-0 p-0.5 hover:bg-slate-200 rounded transition hidden sm:block"
                        title={expandedTasks.has(t.id) ? "Masquer détails" : "Afficher détails"}
                      >
                        {expandedTasks.has(t.id) ? (
                          <ChevronUp className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-500" />
                        ) : (
                          <ChevronDown className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-500" />
                        )}
                      </button>
                      <div className="flex-1 min-w-0">
                        <div className="text-[8px] sm:text-[10px] font-mono font-bold text-slate-600 truncate">{t.id}</div>
                        <div className="text-[10px] sm:text-xs text-slate-700 truncate hidden sm:block">{t.task}</div>
                      </div>
                    </div>
                  ))}
                </div>
